...
export class CirclesController {
  ...
  @Post('create-with-members/:circleOwnerId')
  // @ApiBody({ type: CreateCircleDto, description: 'Data for creating a circle' })
  // @ApiParam({
  //   name: 'circleOwnerId',
  //   description: 'ID of the circle owner',
  //   example: 1,
  // })
  // @ApiCreatedResponse({
  //   description: 'Circle created with members successfully',
  //   type: BaseResponse,
  // })
  // @ApiNotFoundResponse({
  //   description: 'Circle owner not found',
  //   type: BaseResponse,
  // })
  // @ApiBadRequestResponse({
  ...
  //   }
  // }
  @Post('/:circleId/members/:userEmail')
  ...
  async getAllCirclesUserIsMemberOf(@Param('userEmail') userEmail: string) {
    try {
      return await this.circleService.getAllCirclesUserIsMemberOf(userEmail);
    } catch (error) {
      if (error instanceof NotFoundException) {
        throw error;
      }
  ...
  @Get('/owned/:userEmail')
  async getAllCirclesCreatedByUser(@Param('userEmail') userEmail: string) {
    try {
      return await this.circleService.getAllCirclesCreatedByUser(userEmail);
    } catch (error) {
      if (error instanceof NotFoundException) {
        throw error;
      }
